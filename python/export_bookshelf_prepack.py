...
def _render_scl(layout, model_to_resources, models, family, device, package):
    ...
    # Conservative single clock region for iCE40 first version.
    ...
    return OrderedDict(
        [
            ...
            (
                "notes",
                [
                    "Generated by nextpnr pre-pack exporter (iCE40 first version).",
...
def main(context):
    ...
    bel_types = set(arch_data["bel_type_counts"].keys())
    if "ICESTORM_LC" not in bel_types and "SB_IO" not in bel_types:
        raise RuntimeError(
            "This exporter currently supports iCE40 only. "
            "Detected bel types do not look like iCE40 (archId=%s)." % context.archId()
        )
